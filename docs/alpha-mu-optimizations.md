...
| `claim-bounds` | `claim_bounds` | `prove_declarer_claim`, `evaluate_claim_bound` | A DTAC-style search proves an immediate declarer/dummy claim against the combined defender pool without inspecting the E/W split | `claim_probes`, `claim_states`, `claim_cache_hits`, `claim_equivalent_cards_skipped`, `claim_cuts`, `claim_ms` |
